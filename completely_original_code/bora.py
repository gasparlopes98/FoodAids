...
def get_recommendations(N, scores):
    ...
    df_recipes = pd.read_csv("completely_original_code/parsed.csv")
    # order the scores with and filter to get the highest N scores
    top = sorted(range(len(scores)), key=lambda i: scores[i], reverse=True)[:N]
    # create dataframe to load in recommendations
    recommendation = pd.DataFrame(columns=["title", "ingredients", "score"])
    count = 0
    for i in top:
...
def get_recs(ingredients, N=5, mean=False):
    ...
    model = Word2Vec.load("completely_original_code/models/model_cbow.bin")
    model.init_sims(replace=True)
    if model:
        print("Successfully loaded model")
    # load in data
    data = pd.read_csv("completely_original_code/parsed.csv")
    # parse ingredients
